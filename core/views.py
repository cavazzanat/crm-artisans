...
def operation_edit(request, operation_id):
    """Modification d'une opération en direct"""
    operation = get_object_or_404(Operation, id=operation_id, user=request.user)
    ...
    if request.method == 'POST':
        ...
        if not type_prestation:
            ...
            try:
                ...
                operation.date_prevue = None
                # operation.date_realisation = None  # À décommenter après migration
                # operation.date_paiement = None     # À décommenter après migration
                ...
                HistoriqueOperation.objects.create(
                    operation=operation,
                    action="Opération modifiée",
                    utilisateur=request.user
                )
                
                messages.success(request, f"Opération {operation.id_operation} modifiée avec succès !")
            except Exception as e:
